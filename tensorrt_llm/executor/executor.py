...
class GenerationExecutor(ABC):
    ...
    def invalidate_kv_prefix(self, prefix_tokens) -> bool:
        ...
        Default no-op implementation so callers can use the REST endpoint
        safely even on executor backends that don't wire the call through.
        Concrete subclasses that own a pytorch KVCacheManager override this.
